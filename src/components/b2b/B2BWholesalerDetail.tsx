...
export default function B2BWholesalerDetail({ slug }: { slug: string }) {
  ...
  if (!store) return <div className="mx-auto max-w-3xl px-4 py-24 text-center"><h1 className="text-2xl font-black">Mağaza bulunamadı</h1><Link href="/b2b/toptancilar" className="mt-4 inline-block text-sm font-bold text-slate-700 underline">Toptancılara dön</Link></div>;
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <div className="h-48 bg-[#07111F]">{store.cover_url && <img src={store.cover_url} alt="" className="h-full w-full object-cover opacity-90" />}</div>
        <div className="relative p-6 pt-14 sm:p-8 sm:pt-16">
          <div className="absolute -top-12 left-6 flex size-24 items-center justify-center overflow-hidden rounded-2xl border-4 border-white bg-slate-100 text-3xl font-black text-slate-600 shadow-sm sm:left-8">{store.logo_url ? <img src={store.logo_url} alt={`${store.name} logosu`} className="h-full w-full object-cover" /> : store.name.slice(0, 2).toLocaleUpperCase("tr-TR")}</div>
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start"><div><div className="flex flex-wrap items-center gap-2"><h1 className="text-3xl font-black text-slate-950">{store.name}</h1><span className="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 px-2.5 py-1 text-[10px] font-black text-slate-700"><svg aria-hidden="true" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" className="size-3.5"><path d="M4 10.5l4 4 8-9" strokeLinecap="round" strokeLinejoin="round" /></svg>Doğrulanmış toptancı</span></div><p className="mt-2 text-sm font-semibold text-slate-500">{store.city || "Türkiye geneli"} · <span className="text-slate-800">{Number(store.rating).toFixed(1)} / 5 ({store.review_count} değerlendirme)</span></p>{store.description && <p className="mt-4 max-w-3xl text-sm font-medium leading-6 text-slate-600">{store.description}</p>}<button disabled={!verified || chatBusy} onClick={startChat} className="mt-5 rounded-xl bg-slate-900 px-5 py-3 text-xs font-black text-white shadow-sm transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40">{verified ? "Toptancıya mesaj gönder" : "Mesaj için işletmenizi doğrulayın"}</button></div>{store.shipping_terms && <div className="rounded-xl bg-slate-50 p-4 text-xs font-semibold text-slate-600 md:max-w-xs"><span className="block font-black text-slate-900">Teslimat koşulları</span><span className="mt-1 block">{store.shipping_terms}</span></div>}</div>
        </div>
      </section>
      <div className="mb-5 mt-9 flex items-end justify-between"><div><span className="text-xs font-black uppercase tracking-wider text-slate-500">Mağaza kataloğu</span><h2 className="mt-1 text-2xl font-black text-slate-950">Ürünler</h2></div><span className="text-sm font-bold text-slate-500">{products.length} ürün</span></div>
      {products.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-16 text-center text-sm font-semibold text-slate-500">Bu mağazada henüz ürün bulunmuyor.</div> : <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{products.map((product) => <Link key={product.id} href={`/b2b/urun/${product.slug}`} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm hover:shadow-lg"><div className="aspect-[4/3] bg-slate-100">{product.image_urls?.[0] ? <img src={product.image_urls[0]} alt={product.name} className="h-full w-full object-cover transition group-hover:scale-105" /> : <div className="flex h-full items-center justify-center"><svg aria-hidden="true" viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="2" className="size-12 text-slate-300"><path d="M24 6l16 9v18l-16 9-16-9V15z" strokeLinejoin="round" /><path d="M8 15l16 9 16-9M24 24v18" strokeLinejoin="round" /></svg></div>}</div><div className="p-4"><span className="text-[10px] font-bold uppercase text-slate-500">{product.category}</span><h3 className="mt-2 line-clamp-2 text-base font-black text-slate-950">{product.name}</h3><p className="mt-3 text-xs font-semibold text-slate-500">Min. {product.minimum_order_quantity} {product.unit} · {product.vat_included ? "KDV dahil" : "KDV hariç"}</p><strong className="mt-4 block border-t border-slate-100 pt-4 text-sm text-slate-950">{verified && product.price !== undefined ? new Intl.NumberFormat("tr-TR", { style: "currency", currency: product.currency || "TRY" }).format(product.price) : "Fiyat kapalı"}</strong></div></Link>)}</div>}
    </main>
  );
}
