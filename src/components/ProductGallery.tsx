    ...
    <>
      <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {products.map((p, i) => {
          ...
          return (
            <div key={p.id} className="border border-[#E2E8F0] rounded-lg overflow-hidden bg-white group flex flex-col">
              <div className="aspect-[4/3] bg-[#F8FAFC] relative overflow-hidden">
                ...
                />
                {Number(p.price) > 0 ? (
                  <span className="absolute top-2 right-2 bg-white/90 backdrop-blur-sm text-[#0F172A] text-[13px] font-black px-2 py-0.5 rounded shadow-sm">
                    {Number(p.price).toLocaleString("tr-TR")} TL
                  </span>
                ) : (
                  <span className="absolute top-2 right-2 bg-amber-500/90 backdrop-blur-sm text-white text-[10px] font-bold px-1.5 py-0.5 rounded">
                    Fiyat Sorunuz
                  </span>
                )}
              </div>
              <div className="p-4 space-y-2.5 flex flex-1 flex-col">
                <p className="text-sm font-bold text-[#0F172A] line-clamp-1">{p.name}</p>
                {p.description && (
                  <p className="text-xs text-[#0F172A]/55 line-clamp-2">{p.description}</p>
                )}
                <div className="flex-1" />
                {productWhatsapp && (
                  <a
                    href={`https://wa.me/${productWhatsapp.replace(/\D/g, "")}?text=${encodeURIComponent(
                      Number(p.price) > 0
                        ? `${p.name} ürününüzü satın almak istiyorum.`
                        : `${p.name} için fiyat bilgi almak istiyorum.`
                    )}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`mt-2 inline-flex w-full items-center justify-center gap-2 rounded-md px-4 py-3 text-sm font-black transition-all duration-150 ${
                      isPremium
                        ? "bg-gradient-to-r from-[#0F172A] to-[#1E293B] text-white shadow-md shadow-slate-900/15 hover:from-[#1E293B] hover:to-[#334155]"
                  ...
                  >
                    ...
                    {Number(p.price) > 0 ? "Satın Al" : "Fiyat Sor"}
                  </a>
                )}
              </div>
            </div>
