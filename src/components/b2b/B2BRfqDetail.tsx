...
export default function B2BRfqDetail({ rfqId }: { rfqId: string }) {
  ...
  if (notFound || !detail) return <main className="mx-auto max-w-3xl px-4 py-20 text-center">
    ...
    <Link href="/b2b/rfq" className="mt-6 inline-block rounded-xl bg-slate-900 px-5 py-3 text-sm font-black text-white transition hover:bg-slate-800">RFQ tahtasına dön</Link>
  </main>;
  ...
  const statusTone = rfq.status === "open" ? "bg-emerald-50 text-emerald-700" : rfq.status === "awarded" ? "bg-slate-100 text-slate-800" : "bg-slate-100 text-slate-600";
  ...
  return <main className="mx-auto max-w-5xl px-4 py-9">
    ...
    <section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-100 bg-[#07111F] p-6 text-white sm:p-8">
        <div className="flex flex-wrap items-center gap-2.5"><span className="rounded-lg bg-white/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-slate-200">{rfq.category}</span><span className={`rounded-lg px-2.5 py-1 text-[10px] font-black ${statusTone}`}>{B2B_RFQ_STATUS_LABELS[rfq.status] ?? rfq.status}</span>{isOpen && <span className="ml-auto text-[10px] font-bold text-slate-400">Son teklif: {new Date(rfq.valid_until).toLocaleDateString("tr-TR")}</span>}</div>
      ...
      <div className="grid gap-4 p-6 sm:grid-cols-3 sm:p-8">
        ...
        <div className="rounded-xl bg-slate-50 p-4"><span className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-400">Gelen teklifler</span><strong className="mt-1.5 block text-xl font-black text-slate-900">{visibleOffers.length}</strong></div>
      </div>
          ...
          return <article key={offer.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
              <div className="flex items-center gap-3">
                ...
                <div>{store ? <Link href={`/b2b/toptanci/${store.slug}`} className="text-sm font-black text-slate-950 hover:text-slate-700">{store.name}</Link> : <span className="text-sm font-black text-slate-950">Toptancı</span>}{store?.city && <span className="block text-[10px] font-bold text-slate-400">{store.city}{store.rating ? ` · ${Number(store.rating).toFixed(1)}/5 puan` : ""}</span>}</div>
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
              ...
              {isOpen && offer.status === "submitted" && <button disabled={busy === offer.id} onClick={() => acceptOffer(offer.id)} className="rounded-xl bg-slate-900 px-5 py-2.5 text-xs font-black text-white transition hover:bg-slate-800 disabled:opacity-50">Teklifi kabul et</button>}
            </div>
          </article>;
        })}</div>}
    </section>}
    ...
    {!is_mine && <section className="mt-8">
      {myOffer && myOffer.status !== "withdrawn" ? <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6">
        <span className="text-[9px] font-black uppercase tracking-[0.18em] text-slate-500">Senin teklifin</span>
        ...
        {showOfferForm ? <form onSubmit={submitOffer} className="mt-5 grid gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:grid-cols-2">
          <label className="text-xs font-black text-slate-700">Birim fiyat * <span className="font-medium text-slate-400">({rfq.unit} başına)</span><div className="flex gap-2"><input required type="number" min={0.01} step="any" value={offerPrice} onChange={(e) => setOfferPrice(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm font-medium outline-none focus:border-slate-400" /><select value={offerCurrency} onChange={(e) => setOfferCurrency(e.target.value as typeof offerCurrency)} className="mt-1.5 w-20 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-black text-slate-700 outline-none focus:border-slate-400"><option>TRY</option><option>USD</option><option>EUR</option></select></div></label>
          <label className="text-xs font-black text-slate-700">Tedarik süresi (gün) *<input required type="number" min={0} max={365} value={offerLeadTime} onChange={(e) => setOfferLeadTime(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm font-medium outline-none focus:border-slate-400" /></label>
          <label className="text-xs font-black text-slate-700 sm:col-span-2">Koşulların<textarea maxLength={1000} rows={3} value={offerNote} onChange={(e) => setOfferNote(e.target.value)} placeholder="Kdv, ödeme vadeli, karışık set, irsaliyeli teslim…" className="mt-1.5 w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm font-medium outline-none focus:border-slate-400" /></label>
          <div className="sm:col-span-2"><button type="submit" disabled={busy === "offer"} className="rounded-xl bg-slate-900 px-6 py-3 text-xs font-black text-white transition hover:bg-slate-800 disabled:opacity-50">{busy === "offer" ? "Gönderiliyor…" : "Teklifi gönder"}</button></div>
        </form> : <button onClick={() => setShowOfferForm(true)} className="mt-4 rounded-xl bg-slate-900 px-6 py-3 text-sm font-black text-white transition hover:bg-slate-800">Teklif ver</button>}
