      ...
      {ordered.length < 2 ? <p className="mt-8 text-sm font-medium text-slate-500">Grafik için henüz yeterli fiyat değişimi yok.</p> : (
        <>
          <svg viewBox="0 0 600 120" className="mt-5 h-36 w-full" role="img" aria-label="Ürün fiyat geçmişi grafiği">
            ...
            <polyline points={polyline} fill="none" stroke="#0F172A" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
...
export default function B2BProductDetail({ slug }: { slug: string }) {
  ...
  if (!product) return <div className="mx-auto max-w-3xl px-4 py-24 text-center"><h1 className="text-2xl font-black">Ürün bulunamadı</h1><Link href="/b2b" className="mt-4 inline-block text-sm font-bold text-slate-700 underline">Ürünlere dön</Link></div>;

  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      <nav className="mb-5 flex items-center gap-2 overflow-hidden text-[10px] font-black uppercase tracking-[0.14em] text-slate-400"><Link href="/b2b" className="shrink-0 hover:text-slate-700">B2B pazar</Link><span>/</span><span className="shrink-0 text-slate-600">{product.category}</span><span>/</span><span className="truncate text-slate-600">{product.name}</span></nav>
      <div className="grid gap-7 lg:grid-cols-[1.08fr_.92fr]">
        <section className="grid gap-3 sm:grid-cols-[72px_1fr]">
          <div className="order-2 flex gap-2 overflow-x-auto sm:order-1 sm:flex-col">{images.map((image, index) => <button key={image} type="button" onClick={() => setSelectedImage(index)} className={`size-16 shrink-0 overflow-hidden rounded-xl border-2 bg-white ${selectedImage === index ? "border-slate-900" : "border-slate-200 hover:border-slate-400"}`}><img src={image} alt="" className="h-full w-full object-cover" /></button>)}</div>
          <div className="order-1 relative flex aspect-square items-center justify-center overflow-hidden rounded-[1.5rem] border border-slate-200 bg-slate-50 sm:order-2">{images[selectedImage] ? <img src={images[selectedImage]} alt={product.name} className="h-full w-full object-contain p-6" /> : <svg aria-hidden="true" viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="1.5" className="size-24 text-slate-300"><path d="M24 6l16 9v18l-16 9-16-9V15z" strokeLinejoin="round" /><path d="M8 15l16 9 16-9M24 24v18" strokeLinejoin="round" /></svg>}<span className="absolute left-4 top-4 inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[9px] font-black uppercase tracking-wider text-slate-700"><i className="size-1.5 rounded-full bg-emerald-500" />{product.stock_status === "in_stock" ? "Stokta" : product.stock_status === "low_stock" ? "Kritik stok" : product.stock_status === "preorder" ? "Ön sipariş" : "Tükendi"}</span></div>
        </section>
        <section className="rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          <span className="text-xs font-black uppercase tracking-[0.16em] text-slate-500">{product.category}{product.brand ? ` · ${product.brand}` : ""}</span>
          <h1 className="mt-3 text-3xl font-black leading-tight tracking-[-0.035em] text-slate-950 sm:text-4xl">{product.name}</h1>
          <div className="mt-4 flex flex-wrap items-center gap-2"><Link href={`/b2b/toptanci/${product.wholesaler?.slug}`} className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-200"><svg aria-hidden="true" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" className="size-3.5"><path d="M4 10.5l4 4 8-9" strokeLinecap="round" strokeLinejoin="round" /></svg>{product.wholesaler?.name} · {Number(product.wholesaler?.rating ?? 0).toFixed(1)} / 5 puan</Link><B2BFavoriteButton productId={product.id} showLabel /></div>
          {product.description && <p className="mt-5 text-sm font-medium leading-7 text-slate-600">{product.description}</p>}
          <div className="relative mt-6 overflow-hidden rounded-2xl bg-[#07111F] p-5 text-white"><div className="flex items-end justify-between gap-4"><div><span className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Esnafa özel toptan fiyat</span><strong className="mt-2 block text-2xl font-black sm:text-3xl">{verified && product.price !== undefined ? new Intl.NumberFormat("tr-TR", { style: "currency", currency: product.currency || "TRY" }).format(product.price) : "Fiyat erişimi kapalı"}</strong></div><span className={`shrink-0 rounded-lg border border-white/15 px-2.5 py-1.5 text-[9px] font-black uppercase ${verified ? "bg-white/10 text-white" : "bg-white/5 text-slate-300"}`}>{verified ? "Yetkili erişim" : "Kilitli"}</span></div>{!verified && <Link href="/b2b/dogrulama" className="mt-4 inline-block text-xs font-black text-white underline">İşletmenizi doğrulayın</Link>}</div>
          ...
          {verified && <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4"><label className="text-[10px] font-black uppercase tracking-wider text-slate-500">Satıcıya ilk mesajınız<textarea required rows={3} value={purchaseNote} onChange={(event) => setPurchaseNote(event.target.value)} placeholder="Örn. 50 adet için teslim tarihi ve ödeme koşullarını paylaşabilir misiniz?" className="mt-2 w-full resize-none rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium normal-case tracking-normal outline-none focus:border-slate-400" /></label><div className="mt-3 flex gap-3"><div className="relative w-32"><input type="number" inputMode="decimal" min={product.minimum_order_quantity} step="1" value={quantity} onChange={(event) => setQuantity(event.target.value)} onBlur={() => { if (quantity === "" || Number(quantity) < Number(product.minimum_order_quantity)) setQuantity(String(product.minimum_order_quantity)); }} aria-label={`Sipariş miktarı (${product.unit})`} className="h-full w-full rounded-xl border border-slate-200 bg-white px-3 pr-10 text-base font-black outline-none focus:border-slate-400" /><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-slate-400">{product.unit}</span></div><button type="button" disabled={purchaseNote.trim().length < 3 || quantity === ""} onClick={requestQuote} className="min-h-14 flex-1 rounded-xl bg-slate-900 px-5 text-sm font-black text-white shadow-sm transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40">Satın alma görüşmesi başlat</button></div><p className="mt-2 text-[10px] font-semibold text-slate-400">Minimum {product.minimum_order_quantity} {product.unit}. Görüşme açılır; toptancı fiyat ve koşulları sistem içinde gönderir.</p></div>}
          {message && <p className="mt-3 rounded-lg bg-slate-100 p-3 text-xs font-bold text-slate-700">{message}</p>}
          <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 border-t border-slate-100 pt-5 text-[10px] font-black uppercase tracking-wide text-slate-500"><span className="inline-flex items-center gap-1.5"><svg aria-hidden="true" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" className="size-3.5 text-slate-400"><path d="M4 10.5l4 4 8-9" strokeLinecap="round" strokeLinejoin="round" /></svg>Doğrulanmış satıcı</span><span className="inline-flex items-center gap-1.5"><svg aria-hidden="true" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" className="size-3.5 text-slate-400"><path d="M4 10.5l4 4 8-9" strokeLinecap="round" strokeLinejoin="round" /></svg>Kayıtlı fiyat geçmişi</span><span className="inline-flex items-center gap-1.5"><svg aria-hidden="true" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" className="size-3.5 text-slate-400"><path d="M4 10.5l4 4 8-9" strokeLinecap="round" strokeLinejoin="round" /></svg>Güvenli talep akışı</span></div>
        </section>
        {verified && product.price !== undefined && <PriceHistory points={history} currency={product.currency || "TRY"} />}
        <section className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><span className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Ürün bilgileri</span><h2 className="mt-1 text-lg font-black">Teknik özellikler</h2>{specs.length ? <dl className="mt-5 divide-y divide-slate-100">{specs.map(([key, value]) => <div key={key} className="flex justify-between gap-5 py-3 text-sm"><dt className="font-semibold text-slate-500">{key}</dt><dd className="text-right font-bold text-slate-900">{String(value)}</dd></div>)}</dl> : <p className="mt-5 text-sm text-slate-500">Teknik özellikler henüz eklenmemiş.</p>}</section>
      </div>
    </main>
  );
}
