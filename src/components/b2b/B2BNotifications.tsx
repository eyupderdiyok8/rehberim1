...
export default function B2BNotifications() {
  ...
  return <main className="mx-auto max-w-4xl px-4 py-9">
    ...
    {items.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-20 text-center"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="mx-auto size-10 text-slate-300"><path d="M4 12.5l5 5L20 6.5" /></svg><h2 className="mt-4 text-lg font-black text-slate-800">Her şey güncel</h2><p className="mt-2 text-sm text-slate-500">Yeni bir mesaj veya işlem olduğunda burada göreceksiniz.</p></div> : <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">{items.map((item) => <article key={item.id} className={`flex gap-4 border-b border-slate-100 p-5 last:border-0 ${item.read_at ? "bg-white" : "bg-slate-50"}`}><span className={`mt-1 grid size-10 shrink-0 place-items-center rounded-xl ${item.read_at ? "bg-slate-100 text-slate-500" : "bg-slate-900 text-white"}`}>{item.kind === "message" ? <svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" className="size-[18px]"><rect x="3" y="5" width="18" height="14" rx="2" /><path d="m3.5 7 8.5 6 8.5-6" /></svg> : item.kind === "verification" ? <svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-[18px]"><path d="M4 12.5l5 5L20 6.5" /></svg> : item.kind === "price" ? <svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" className="size-[18px]"><path d="M4 6l7 7 3-3 6 6" /><path d="M20 16h-4v-4" /></svg> : <svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" className="size-[18px]"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" /><path d="M13.7 21a2 2 0 0 1-3.4 0" /></svg>}</span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><strong className="text-sm font-black text-slate-950">{item.title}</strong><time className="shrink-0 text-[10px] font-bold text-slate-400">{new Date(item.created_at).toLocaleDateString("tr-TR", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</time></div><p className="mt-1 text-sm font-medium leading-6 text-slate-600">{item.body}</p><div className="mt-3 flex gap-3">{item.href && <Link href={item.href} onClick={() => !item.read_at && markRead(item.id)} className="text-xs font-black text-slate-900 underline underline-offset-2">Aç</Link>}{!item.read_at && <button onClick={() => markRead(item.id)} className="text-xs font-black text-slate-500">Okundu</button>}</div></div></article>)}</div>}
  </main>;
}
