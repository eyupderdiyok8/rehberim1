...
export default function B2BAdminPage() {
  ...
  return (
    <div className="space-y-7">
      ...
      <form onSubmit={createWholesaler} className="rounded-xl border border-slate-200 bg-slate-50 p-5"><div className="mb-4"><h2 className="text-base font-black text-slate-950">Toptancı mağazası aç</h2><p className="mt-1 text-xs font-medium text-slate-600">Kullanıcının kimliğini, mağaza adını ve bağlantı adresini girin. Profil sadece giriş yapanlara açılır.</p></div><div className="grid gap-3 lg:grid-cols-[1fr_1fr_1fr_auto]"><input required value={wholesaler.ownerId} onChange={(e)=>setWholesaler({...wholesaler,ownerId:e.target.value})} placeholder="Kullanıcı UUID" className="rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm" /><input required value={wholesaler.name} onChange={(e)=>setWholesaler({...wholesaler,name:e.target.value})} placeholder="Mağaza adı" className="rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm" /><input required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" value={wholesaler.slug} onChange={(e)=>setWholesaler({...wholesaler,slug:e.target.value.toLowerCase()})} placeholder="magaza-adresi" className="rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm" /><button disabled={busyId==="wholesaler"} className="rounded-lg bg-slate-900 px-4 py-2.5 text-xs font-black text-white transition hover:bg-slate-800 disabled:opacity-50">Mağazayı aç</button></div></form>
