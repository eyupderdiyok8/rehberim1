...
export default function AdminB2BWholesalersPage() {
  ...
  return <div className="space-y-7">
    <header className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end"><div><span className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">B2B operasyon</span><h1 className="mt-2 text-3xl font-black text-slate-950">Toptancı yönetimi</h1><p className="mt-2 text-sm font-medium text-slate-500">Mağazaları, hesap erişimini ve katalog durumunu tek merkezden yönetin.</p></div><button onClick={() => setShowCreate((value) => !value)} className="w-fit rounded-xl bg-slate-900 px-5 py-3 text-sm font-black text-white shadow-sm transition hover:bg-slate-800">{showCreate ? "Formu kapat" : "Yeni toptancı"}</button></header>
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
      ["Toplam toptancı", stores.length, "bg-slate-100 text-slate-700"],
      ["Aktif mağaza", activeCount, "bg-emerald-50 text-emerald-700"],
      ["Askıya alınan", suspendedCount, "bg-red-50 text-red-700"],
      ["Toplam ürün", products.length, "bg-slate-900 text-white"],
    ...
    {showCreate && <form onSubmit={createStore} className="rounded-2xl border border-slate-200 bg-slate-50 p-6"><h2 className="text-lg font-black text-slate-950">Yeni toptancı mağazası</h2><p className="mt-1 text-xs font-medium text-slate-600">Önce kayıtlı kullanıcıyı seçin; hesap otomatik olarak toptancı rolüne geçirilir.</p><div className="mt-5 grid gap-3 lg:grid-cols-[1.2fr_1fr_1fr_auto]"><select required value={createForm.ownerId} onChange={(event) => { const ownerId = event.target.value; const member = memberById.get(ownerId); setCreateForm({ ...createForm, ownerId, name: createForm.name || member?.business_name || "" }); }} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm"><option value="">Kayıtlı kullanıcı seçin</option>{unassignedMembers.map((member) => <option key={member.user_id} value={member.user_id}>{member.business_name || member.user_id} · {member.city || "Şehir yok"}</option>)}</select><input required value={createForm.name} onChange={(event) => setCreateForm({...createForm,name:event.target.value})} placeholder="Mağaza adı" className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm" /><input required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" value={createForm.slug} onChange={(event) => setCreateForm({...createForm,slug:event.target.value.toLowerCase()})} placeholder="magaza-adresi" className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm" /><button disabled={busy === "create"} className="rounded-xl bg-slate-900 px-5 py-3 text-xs font-black text-white transition hover:bg-slate-800 disabled:opacity-50">Mağazayı oluştur</button></div></form>}

    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="grid gap-3 lg:grid-cols-[1fr_auto]"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Mağaza, şehir veya işletme ara…" className="min-h-12 rounded-xl border border-slate-200 bg-slate-50 px-4 text-sm outline-none focus:border-slate-400" /><div className="flex gap-2">{(["all","active","passive"] as Filter[]).map((item) => <button key={item} onClick={() => setFilter(item)} className={`rounded-xl px-4 py-2 text-xs font-black ${filter === item ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>{item === "all" ? "Tümü" : item === "active" ? "Aktif" : "Pasif"}</button>)}</div></div></section>
