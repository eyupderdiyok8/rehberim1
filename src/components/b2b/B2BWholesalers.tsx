    ...
    <main className="mx-auto max-w-7xl px-4 py-9">
      <section className="relative mb-8 overflow-hidden rounded-[2rem] bg-[#07111F] text-white shadow-2xl shadow-slate-950/10">
        ...
        <div className="relative grid gap-6 p-7 sm:p-10 lg:grid-cols-[1.2fr_.8fr] lg:items-end">
          <div>
            <span className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-400">Tedarikçi ağı / Türkiye</span>
            ...
            <div className="relative mt-7 max-w-xl">
              <svg aria-hidden="true" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.8" className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-slate-400"><circle cx="9" cy="9" r="5.5" /><path d="M13 13l4 4" strokeLinecap="round" /></svg>
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Toptancı, şehir veya uzmanlık ara…" className="min-h-14 w-full rounded-2xl border border-white/15 bg-white/[.06] pl-11 pr-4 text-base font-semibold text-white outline-none transition placeholder:text-slate-500 focus:border-white/35 focus:bg-white/10" />
            </div>
          </div>
      ...
      <div className="mb-5 flex items-end justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Tedarikçi dizini</p><h2 className="mt-1 text-xl font-black text-slate-950">{filtered.length} mağaza listeleniyor</h2></div><span className="hidden text-xs font-bold text-slate-400 sm:block">İtibar puanına göre sıralı</span></div>
      {error ? <div className="rounded-xl border border-red-200 bg-red-50 p-5 text-sm font-semibold text-red-700">{error}</div> : filtered.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-20 text-center text-sm font-semibold text-slate-500">Henüz aktif toptancı mağazası bulunmuyor.</div> : (
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((store) => (
            <Link key={store.id} href={`/b2b/toptanci/${store.slug}`} className="group overflow-hidden rounded-[1.4rem] border border-slate-200 bg-white shadow-sm transition hover:border-slate-300 hover:shadow-md">
              <div className="relative h-32 overflow-hidden bg-[#07111F]">{store.cover_url && <img src={store.cover_url} alt="" className="h-full w-full object-cover opacity-85 transition duration-500 group-hover:scale-105" />}<span className="absolute right-4 top-4 rounded-md border border-white/20 bg-white/10 px-2.5 py-1 text-[9px] font-black uppercase tracking-wider text-white">B2B verified</span></div>
              <div className="relative p-5 pt-10">
                <div className="absolute -top-8 left-5 flex size-16 items-center justify-center overflow-hidden rounded-xl border-4 border-white bg-slate-100 text-xl font-black text-slate-600 shadow-sm">{store.logo_url ? <img src={store.logo_url} alt={`${store.name} logosu`} className="h-full w-full object-cover" /> : store.name.slice(0, 2).toLocaleUpperCase("tr-TR")}</div>
                <div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-black text-slate-950 group-hover:text-slate-700">{store.name}</h2><p className="text-xs font-semibold text-slate-500">{store.city || "Türkiye geneli"}</p></div><span className="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 px-2.5 py-1 text-[10px] font-black text-slate-700"><svg aria-hidden="true" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" className="size-3.5"><path d="M4 10.5l4 4 8-9" strokeLinecap="round" strokeLinejoin="round" /></svg>Doğrulandı</span></div>
                {store.description && <p className="mt-4 line-clamp-2 text-sm leading-6 text-slate-600">{store.description}</p>}
                <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4 text-xs"><strong className="text-slate-900">{Number(store.rating).toFixed(1)} / 5 <span className="font-medium text-slate-400">({store.review_count} değerlendirme)</span></strong><span className="font-black text-slate-950 transition group-hover:text-slate-700">Profili incele</span></div>
              </div>
            </Link>
          ))}
        </div>
