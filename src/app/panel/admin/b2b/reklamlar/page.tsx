...
export default function AdminB2BAdsPage() {
  ...
  return <div className="space-y-6"><header><span className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">B2B gelir yönetimi</span><h1 className="mt-2 text-3xl font-black text-slate-950">Reklam onayları</h1><p className="mt-2 text-sm font-medium text-slate-500">Bildirim ve popup kampanyalarını yayına almadan önce inceleyin.</p></header>
    ...
    <div className="flex gap-2 overflow-x-auto">{["all","pending","approved","active","paused","rejected"].map((item)=><button key={item} onClick={()=>setFilter(item)} className={`shrink-0 rounded-xl px-4 py-2.5 text-xs font-black ${filter===item?"bg-slate-900 text-white":"border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"}`}>{item === "all" ? "Tümü" : item}</button>)}</div>
    <section className="space-y-4">{filtered.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-20 text-center text-sm font-semibold text-slate-500">Bu durumda kampanya yok.</div> : filtered.map((ad)=><article key={ad.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="grid md:grid-cols-[180px_1fr]"><div className="grid min-h-36 place-items-center bg-[#07111F] text-white">{ad.image_url ? <img src={ad.image_url} alt="" className="h-full w-full object-cover" /> : <div className="text-center"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="mx-auto size-8 text-slate-400"><path d="M3 11v2a1 1 0 0 0 1 1h3l5 4V6L7 10H4a1 1 0 0 0-1 1z" /><path d="M16 9a4 4 0 0 1 0 6" /></svg><span className="mt-2 block text-[9px] font-black uppercase tracking-wider text-slate-400">Bildirim reklam</span></div>}</div><div className="p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row"><div><span className="text-[9px] font-black uppercase tracking-wider text-slate-500">{storeNames.get(ad.wholesaler_id) || "Toptancı"} · {ad.ad_type}</span><h2 className="mt-1 text-lg font-black text-slate-950">{ad.title}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{ad.body}</p></div><span className="h-fit rounded-lg bg-slate-100 px-3 py-2 text-xs font-black text-slate-600">{ad.status}</span></div><div className="mt-4 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-4"><span className="mr-auto text-[10px] font-bold text-slate-400">{new Date(ad.starts_at).toLocaleDateString("tr-TR")} – {new Date(ad.ends_at).toLocaleDateString("tr-TR")} · {ad.impressions} gösterim · {ad.clicks} tıklama</span><button disabled={busy===ad.id} onClick={()=>update(ad,"approved")} className="rounded-lg bg-slate-100 px-3 py-2 text-xs font-black text-slate-700 transition hover:bg-slate-200 disabled:opacity-50">Onayla</button><button disabled={busy===ad.id} onClick={()=>update(ad,"active")} className="rounded-lg bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700">Yayına al</button><button disabled={busy===ad.id} onClick={()=>update(ad,"paused")} className="rounded-lg bg-amber-50 px-3 py-2 text-xs font-black text-amber-700">Duraklat</button><button disabled={busy===ad.id} onClick={()=>update(ad,"rejected")} className="rounded-lg bg-red-50 px-3 py-2 text-xs font-black text-red-700">Reddet</button></div></div></div></article>)}</section>
  </div>;
}
