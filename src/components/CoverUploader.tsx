...
export default function CoverUploader({ currentUrl, onUpload, firmName }: Props) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const prefix = firmName ? turkishToSlug(firmName) : "cover";
      ...
      const { data } = supabase.storage.from("firm-covers").getPublicUrl(path);
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        ...
        className={`relative flex items-center justify-center w-full h-48 border-2 border-dashed rounded-xl transition-all cursor-pointer select-none overflow-hidden
      ...
      >
        {preview ? (
          ...
          <div className="flex flex-col items-center gap-2 text-[#0F172A]/40 pointer-events-none p-4 text-center">
            ...
            <p className="text-sm font-semibold">Kapak görseli yüklemek için tıklayın</p>
            <p className="text-xs">Geniş ve yatay bir görsel tercih edin (JPG, PNG, WEBP · max 5 MB)</p>
          </div>
        )}
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="px-4 py-2 text-xs font-bold bg-[#0F172A] hover:bg-[#1E293B] text-white rounded-lg transition-colors disabled:opacity-50"
        >
          {uploading ? "Yükleniyor…" : preview ? "Değiştir" : "Görsel Yükle"}
        </button>
        {preview && (
          <button
            type="button"
