...
export default function B2BAdsManager() {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-9">
    <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><span className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Satıcı büyüme merkezi</span><h1 className="mt-2 text-3xl font-black text-slate-950">B2B reklamları</h1><p className="mt-2 text-sm font-medium text-slate-500">Sadece giriş yapmış sektör profesyonellerine görünür, ölçülebilir kampanyalar oluşturun.</p></div><div className="flex gap-2"><Link href="/b2b/mesajlar" className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-xs font-black text-slate-700">Mesajlar</Link><Link href="/b2b/toptanci-paneli" className="rounded-xl bg-slate-900 px-4 py-3 text-xs font-black text-white transition hover:bg-slate-800">Satıcı merkezi</Link></div></div>
    ...
    <div className="mb-7 grid gap-4 md:grid-cols-2"><div className={`rounded-2xl border p-5 ${form.ad_type === "notification" ? "border-slate-900 bg-slate-50" : "border-slate-200 bg-white"}`}><button onClick={() => setForm({...form,ad_type:"notification"})} className="w-full text-left"><span className="text-[10px] font-black uppercase tracking-wider text-slate-500">Düşük dikkat kesintisi</span><h2 className="mt-2 text-lg font-black">Bildirim reklam</h2><p className="mt-2 text-sm leading-6 text-slate-600">Ekranın köşesinde kompakt kampanya kartı. Ürün lansmanı ve stok fırsatları için.</p></button></div><div className={`rounded-2xl border p-5 ${form.ad_type === "popup" ? "border-slate-900 bg-slate-50" : "border-slate-200 bg-white"}`}><button onClick={() => setForm({...form,ad_type:"popup"})} className="w-full text-left"><span className="text-[10px] font-black uppercase tracking-wider text-slate-500">Yüksek görünürlük</span><h2 className="mt-2 text-lg font-black">Popup reklam</h2><p className="mt-2 text-sm leading-6 text-slate-600">Oturum başına bir kez açılan görselli vitrin. Büyük kampanya ve yeni seri tanıtımı için.</p></button></div></div>
    <div className="grid gap-7 xl:grid-cols-[.9fr_1.1fr]"><form onSubmit={createAd} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-lg font-black">Yeni {form.ad_type === "popup" ? "popup" : "bildirim"} kampanyası</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-xs font-bold text-slate-600 sm:col-span-2">Başlık<input required minLength={3} maxLength={90} value={form.title} onChange={(e)=>setForm({...form,title:e.target.value})} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-slate-400" /></label><label className="text-xs font-bold text-slate-600 sm:col-span-2">Reklam metni<textarea required minLength={3} maxLength={240} rows={4} value={form.body} onChange={(e)=>setForm({...form,body:e.target.value})} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-slate-400" /></label>{form.ad_type === "popup" && <div className="sm:col-span-2"><span className="mb-1.5 block text-xs font-bold text-slate-600">Kampanya görseli</span><B2BImageUploader max={1} value={images} onChange={setImages} /></div>}<label className="text-xs font-bold text-slate-600">Buton metni<input required value={form.cta_label} onChange={(e)=>setForm({...form,cta_label:e.target.value})} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-slate-400" /></label><label className="text-xs font-bold text-slate-600">Hedef bağlantı<input required value={form.target_url} onChange={(e)=>setForm({...form,target_url:e.target.value})} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-slate-400" /></label><label className="text-xs font-bold text-slate-600">Başlangıç<input required type="datetime-local" value={form.starts_at} onChange={(e)=>setForm({...form,starts_at:e.target.value})} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-slate-400" /></label><label className="text-xs font-bold text-slate-600">Bitiş<input required type="datetime-local" value={form.ends_at} onChange={(e)=>setForm({...form,ends_at:e.target.value})} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-slate-400" /></label><label className="text-xs font-bold text-slate-600">Günlük bütçe (₺)<input min="0" step="0.01" type="number" value={form.daily_budget} onChange={(e)=>setForm({...form,daily_budget:e.target.value})} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-slate-400" /></label><label className="text-xs font-bold text-slate-600">Toplam bütçe (₺)<input min="0" step="0.01" type="number" value={form.total_budget} onChange={(e)=>setForm({...form,total_budget:e.target.value})} className="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-slate-400" /></label></div><button disabled={busy} className="mt-5 w-full rounded-xl bg-slate-900 px-5 py-3.5 text-sm font-black text-white transition hover:bg-slate-800 disabled:opacity-50">Onaya gönder</button></form>
      <section><div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-black">Kampanyalarım</h2><span className="rounded-lg bg-slate-100 px-3 py-2 text-xs font-black text-slate-600">{ads.length} kampanya</span></div>{ads.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-20 text-center text-sm font-semibold text-slate-500">Henüz kampanya oluşturmadınız.</div> : <div className="space-y-3">{ads.map((ad)=><article key={ad.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><span className="text-[9px] font-black uppercase tracking-wider text-slate-500">{ad.ad_type === "popup" ? "Popup" : "Bildirim"}</span><h3 className="mt-1 font-black text-slate-950">{ad.title}</h3></div><span className="rounded-lg bg-slate-100 px-2.5 py-1.5 text-[9px] font-black text-slate-600">{statusLabel[ad.status] || ad.status}</span></div><p className="mt-3 line-clamp-2 text-xs leading-5 text-slate-500">{ad.body}</p><div className="mt-4 grid grid-cols-3 gap-2 border-t border-slate-100 pt-4"><div><strong className="block text-lg font-black">{ad.impressions}</strong><span className="text-[9px] font-bold uppercase text-slate-400">Gösterim</span></div><div><strong className="block text-lg font-black">{ad.clicks}</strong><span className="text-[9px] font-bold uppercase text-slate-400">Tıklama</span></div><div><strong className="block text-lg font-black">{ad.impressions ? `%${((ad.clicks/ad.impressions)*100).toFixed(1)}` : "%0"}</strong><span className="text-[9px] font-bold uppercase text-slate-400">CTR</span></div></div>{ad.admin_note && <p className="mt-3 rounded-lg bg-amber-50 p-3 text-xs font-semibold text-amber-800">Yönetici notu: {ad.admin_note}</p>}</article>)}</div>}</section></div>
  </main>;
}
