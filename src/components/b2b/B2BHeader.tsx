  ...
  return <>
    ...
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
      <div className="mx-auto flex min-h-16 max-w-7xl items-center gap-5 px-4">
        ...
        {email && <nav className="hidden items-center gap-1 rounded-xl bg-slate-100/80 p-1 lg:flex">{navItems.map((item) => { const active = item.href === "/b2b" ? pathname === item.href : pathname.startsWith(item.href); return <Link key={item.href} href={item.href} className={`rounded-lg px-3.5 py-2 text-xs font-black transition ${active ? "bg-white text-slate-950 shadow-sm" : "text-slate-500 hover:text-slate-900"}`}>{item.label}</Link>; })}{(accountType === "wholesaler" || accountType === "admin") && <Link href="/b2b/toptanci-paneli" className={`rounded-lg px-3.5 py-2 text-xs font-black ${pathname.startsWith("/b2b/toptanci-paneli") ? "bg-violet-600 text-white shadow-sm" : "text-violet-700 hover:bg-violet-50"}`}>Satıcı merkezi</Link>}</nav>}
