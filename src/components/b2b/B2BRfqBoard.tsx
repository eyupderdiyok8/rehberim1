  ...
  return <main className="mx-auto max-w-7xl px-4 py-8">
    <section className="relative mb-8 overflow-hidden rounded-[2rem] bg-[#07111F] text-white shadow-2xl shadow-slate-950/15">
      ...
      <div className="relative grid gap-8 p-6 sm:p-10 lg:grid-cols-[1.1fr_.9fr] lg:items-center lg:p-12">
        <div>
          <span className="inline-flex items-center gap-2 rounded-full border border-white/15 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-300"><i className="size-1.5 rounded-full bg-white" /> RFQ tahtası</span>
          <h1 className="mt-5 text-3xl font-black leading-[1.06] tracking-[-0.03em] sm:text-5xl">Aradığın ürünü<br className="hidden sm:block" /> toptancılar sana bulsun.</h1>
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            {verified && <Link href="/b2b/rfq/olustur" className="rounded-xl bg-white px-5 py-3 text-xs font-black text-slate-950 transition hover:bg-slate-100">Satın alma ilanı aç</Link>}
            {!verified && <Link href="/b2b/dogrulama" className="rounded-xl bg-white px-5 py-3 text-xs font-black text-slate-950 transition hover:bg-slate-100">İlan açmak için doğrulan</Link>}
            <a href="#ilanlar" className="rounded-xl border border-white/25 px-5 py-3 text-xs font-black text-white transition hover:bg-white/10">Açık ilanları gör</a>
          </div>
        </div>
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 shadow-2xl">
          <div className="flex items-center justify-between border-b border-white/10 pb-4"><div><span className="text-[9px] font-black uppercase tracking-[0.18em] text-slate-400">Tahtanın nabzı</span><h2 className="mt-1 text-sm font-black">Bugünün talep akışı</h2></div>{wholesaler && <Link href="/b2b/toptanci-paneli?bolum=rfq" className="rounded-lg bg-white/10 px-3 py-2 text-[9px] font-black text-white transition hover:bg-white/15">Teklif ver</Link>}</div>
    ...
    <div id="ilanlar" className="mb-6 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:flex-row lg:items-center">
      <div className="relative min-w-0 flex-1"><svg aria-hidden="true" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.8" className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-slate-400"><circle cx="9" cy="9" r="5.5" /><path d="M13 13l4 4" strokeLinecap="round" /></svg><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="İlan, kategori, şehir veya esnaf adı ara…" className="w-full rounded-xl border border-slate-200 bg-slate-50/60 py-2.5 pl-10 pr-3 text-sm font-medium outline-none transition focus:border-slate-400 focus:bg-white" /></div>
      <div className="flex flex-wrap items-center gap-2">
        <select value={category} onChange={(e) => setCategory(e.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-black text-slate-700 outline-none focus:border-slate-400">{categories.map((item) => <option key={item}>{item}</option>)}</select>
        <select value={sort} onChange={(e) => setSort(e.target.value as typeof sort)} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-black text-slate-700 outline-none focus:border-slate-400"><option value="newest">En yeni</option><option value="deadline">Süresi azalan</option><option value="offers">Teklif sayısı</option></select>
      </div>
    </div>
    ...
    {loading ? <div className="py-24 text-center text-sm font-bold text-slate-500">Satın alma ilanları hazırlanıyor…</div> : filtered.length === 0 ?
      <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-20 text-center">
        ...
        {verified && <Link href="/b2b/rfq/olustur" className="mt-5 inline-block rounded-xl bg-slate-900 px-5 py-3 text-sm font-black text-white transition hover:bg-slate-800">İlan aç</Link>}
      </div> :
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.map((rfq) => {
        const deadline = deadlineInfo(rfq.valid_until);
        return <Link key={rfq.id} href={`/b2b/rfq/${rfq.id}`} className="group flex flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-slate-300 hover:shadow-md">
          <div className="flex items-start justify-between gap-3">
            ...
            {rfq.status === "awarded" ? <span className="rounded-lg bg-slate-100 px-2.5 py-1 text-[10px] font-black text-slate-700">{B2B_RFQ_STATUS_LABELS.awarded}</span> : <span className={`rounded-lg px-2.5 py-1 text-[10px] font-black ${deadline.tone}`}>{deadline.label}</span>}
          </div>
          <h2 className="mt-3 line-clamp-2 text-base font-black leading-snug text-slate-950 transition group-hover:text-slate-700">{rfq.title}</h2>
          ...
          <div className="mt-auto flex items-center justify-between gap-3 border-t border-slate-100 pt-4 text-[11px] font-bold text-slate-500">
            ...
            <span className={`shrink-0 rounded-lg px-2.5 py-1 font-black ${rfq.offer_count > 0 ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-500"}`}>{rfq.offer_count} teklif</span>
          </div>
          {rfq.my_offer && <span className="mt-2.5 rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-center text-[10px] font-black text-slate-700">Bu ilana teklif verdin</span>}
        </Link>;
      })}</div>}
  </main>;
}
