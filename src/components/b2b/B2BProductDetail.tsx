    ...
    <main className="mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="grid gap-7 lg:grid-cols-[1.08fr_.92fr]">
        ...
        <section className="rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <Link href={`/b2b/toptanci/${product.wholesaler?.slug}`} className="mt-4 inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-2 text-xs font-black text-slate-700 hover:bg-sky-50 hover:text-sky-700"><span className="grid size-5 place-items-center rounded-full bg-slate-950 text-[8px] text-white">✓</span>{product.wholesaler?.name} · ★ {Number(product.wholesaler?.rating ?? 0).toFixed(1)}</Link>
