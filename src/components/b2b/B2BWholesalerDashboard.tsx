...
export default function B2BWholesalerDashboard() {
  ...
  if (!store) return <main className="mx-auto max-w-3xl px-4 py-16"><div className="rounded-2xl border border-slate-200 bg-white p-8"><h1 className="text-xl font-black">Hesabınız hazır, mağaza ataması bekleniyor</h1><p className="mt-2 text-sm text-slate-600">Yönetici mağazanızı oluşturduğunda ürün ve fiyat yönetimi burada açılacak.</p></div></main>;

  return <main className="mx-auto max-w-7xl px-4 py-8">
    <div className="mb-8 flex flex-col justify-between gap-4 rounded-2xl bg-[#07111F] p-7 text-white sm:flex-row sm:items-end"><div><span className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Toptancı çalışma alanı</span><h1 className="mt-2 text-3xl font-black">{store.name}</h1><p className="mt-2 text-sm text-slate-300">Mağaza, ürün, ticaret görüşmeleri ve kampanyaları tek yerden yönetin.</p></div><div className="flex flex-wrap gap-2"><Link href="/b2b/mesajlar" className="rounded-lg border border-white/25 px-3 py-2 text-xs font-black text-white transition hover:bg-white/10">Mesajlar</Link><Link href="/b2b/reklamlar" className="rounded-lg bg-white px-3 py-2 text-xs font-black text-slate-950 transition hover:bg-slate-100">Reklam merkezi</Link><span className={`w-fit rounded-lg border px-3 py-2 text-xs font-black ${store.is_active ? "border-emerald-400/30 bg-emerald-400/10 text-emerald-200" : "border-amber-400/30 bg-amber-400/10 text-amber-200"}`}>{store.is_active ? "Mağaza aktif" : "Yönetici onayı bekliyor"}</span></div></div>
    ...
    ] as [DashboardView,string][]).map(([value,label])=><button key={value} onClick={()=>setActiveView(value)} className={`flex min-h-12 shrink-0 items-center gap-2 rounded-xl px-4 text-xs font-black transition ${activeView===value?"bg-slate-900 text-white shadow-sm":"text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}>{label}{value==="trades"&&requests.filter((request)=>request.status==="requested").length>0&&<span className="rounded-full bg-slate-200 px-2 py-0.5 text-[9px] text-slate-900">{requests.filter((request)=>request.status==="requested").length}</span>}{value==="rfq"&&rfqPool.filter((row)=>row.status==="open"&&!row.my_offer_status).length>0&&<span className="rounded-full bg-slate-200 px-2 py-0.5 text-[9px] text-slate-900">{rfqPool.filter((row)=>row.status==="open"&&!row.my_offer_status).length}</span>}</button>)}</nav>

    {activeView === "overview" && <div className="space-y-6"><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
      [products.filter((item)=>item.is_active).length,"Yayındaki ürün","text-slate-700 bg-slate-100"],
      ...
      [requests.filter((request)=>request.status==="completed").length,"Tamamlanan","text-slate-800 bg-slate-100"],
      ...
      <section className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]"><div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-lg font-black">Son görüşmeler</h2><p className="mt-1 text-xs font-medium text-slate-500">Öncelikli ticaret hareketleri</p></div><button onClick={()=>setActiveView("trades")} className="text-xs font-black text-slate-900 hover:underline">Tümünü aç</button></div>{requests.length===0?<div className="rounded-xl border border-dashed border-slate-200 py-12 text-center text-sm font-semibold text-slate-400">Henüz görüşme yok.</div>:<div className="divide-y divide-slate-100">{requests.slice(0,4).map((request)=><button key={request.id} onClick={()=>setActiveView("trades")} className="flex w-full items-center justify-between gap-4 py-3 text-left"><div className="min-w-0"><strong className="block truncate text-sm text-slate-900">{request.product_name}</strong><span className="mt-1 block truncate text-xs font-semibold text-slate-400">{request.buyer_business_name} · {request.quantity} {request.unit}</span></div><span className="shrink-0 rounded-lg bg-slate-100 px-2.5 py-1.5 text-[9px] font-black text-slate-600">{B2B_STATUS_LABELS[request.status] ?? request.status}</span></button>)}</div>}</div>
        <div className="rounded-2xl bg-[#07111F] p-6 text-white"><span className="text-[9px] font-black uppercase tracking-[0.18em] text-slate-400">Hızlı işlemler</span><h2 className="mt-2 text-xl font-black">Bugün ne yapacaksınız?</h2><div className="mt-6 grid gap-2"><button onClick={()=>setActiveView("products")} className="flex items-center justify-between rounded-xl border border-white/15 px-4 py-3 text-left text-xs font-black transition hover:bg-white/10"><span>Yeni ürün ekle</span><span><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-4"><path d="M4 12h12" /><path d="m11 7 5 5-5 5" /></svg></span></button><button onClick={()=>setActiveView("trades")} className="flex items-center justify-between rounded-xl border border-white/15 px-4 py-3 text-left text-xs font-black transition hover:bg-white/10"><span>Bekleyen görüşmeleri yanıtla</span><span><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-4"><path d="M4 12h12" /><path d="m11 7 5 5-5 5" /></svg></span></button><button onClick={()=>setActiveView("rfq")} className="flex items-center justify-between rounded-xl border border-white/15 px-4 py-3 text-left text-xs font-black transition hover:bg-white/10"><span>RFQ havuzundan teklif ver</span><span><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-4"><path d="M4 12h12" /><path d="m11 7 5 5-5 5" /></svg></span></button><Link href="/b2b/reklamlar" className="flex items-center justify-between rounded-xl bg-white px-4 py-3 text-xs font-black text-slate-950 transition hover:bg-slate-100"><span>Kampanya oluştur</span><span><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-4"><path d="M4 12h12" /><path d="m11 7 5 5-5 5" /></svg></span></Link><button onClick={()=>setActiveView("store")} className="flex items-center justify-between rounded-xl border border-white/15 px-4 py-3 text-left text-xs font-black transition hover:bg-white/10"><span>Mağaza profilini düzenle</span><span><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-4"><path d="M4 12h12" /><path d="m11 7 5 5-5 5" /></svg></span></button></div></div></section>
    </div>}
    ...
    {activeView === "store" && <div className="max-w-3xl">
      ...
      </div><button disabled={busy === "profile"} className="mt-5 rounded-xl bg-slate-900 px-5 py-3 text-sm font-black text-white transition hover:bg-slate-800 disabled:opacity-50">{busy === "profile" ? "Kaydediliyor…" : "Profili kaydet"}</button></form></div>}

    {activeView === "products" && <div className="space-y-6">
      <form ref={productFormRef} onSubmit={saveProduct} className="scroll-mt-32 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><span className="text-[10px] font-black uppercase tracking-wider text-slate-500">{editingProductId ? "Ürün düzenleme" : "Yeni kayıt"}</span><h2 className="mt-1 text-lg font-black">{editingProductId ? "Ürün bilgilerini güncelle" : "Yeni ürün ekle"}</h2><p className="mt-1 text-xs font-medium text-slate-500">Fiyat yalnızca doğrulanmış esnafa ve size görünür.</p></div>{editingProductId && <button type="button" onClick={() => { setEditingProductId(null); setProduct(blankProduct); setProductImages([]); }} className="w-fit rounded-xl bg-slate-100 px-4 py-2.5 text-xs font-black text-slate-600">Düzenlemeyi kapat</button>}</div><div className="grid gap-4 sm:grid-cols-2">
      ...
      </div><div className="mt-5 flex flex-wrap gap-2"><button type="submit" name="visibility" value="publish" disabled={busy === "product"} className="rounded-xl bg-slate-900 px-5 py-3 text-sm font-black text-white transition hover:bg-slate-800 disabled:opacity-50">{busy === "product" ? "Kaydediliyor…" : editingProductId ? "Kaydet ve yayınla" : "Ürünü yayına al"}</button><button type="submit" name="visibility" value="draft" disabled={busy === "product"} className="rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-700 disabled:opacity-50">Taslak kaydet</button></div></form>
