...
export default function B2BHeader() {
  ...
  return <>
    ...
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
      ...
      {email && <nav className="mx-auto flex max-w-7xl gap-2 overflow-x-auto border-t border-slate-100 px-4 py-2 xl:hidden">{navItems.map((item) => <Link key={item.href} href={item.href} className={`shrink-0 rounded-lg px-3 py-2 text-xs font-black ${pathname === item.href ? "bg-slate-950 text-white" : "bg-slate-100 text-slate-600"}`}>{item.label}{item.href === "/b2b/mesajlar" && messageCount > 0 ? ` (${messageCount})` : ""}</Link>)}{(accountType === "wholesaler" || accountType === "admin") && <Link href="/b2b/toptanci-paneli" className="shrink-0 rounded-lg bg-violet-50 px-3 py-2 text-xs font-black text-violet-700">Satıcı merkezi</Link>}</nav>}
    </header>
  </>;
}
